...
def fit_gaussian(embedding_samples, embedding_means):
    sample_covariances = (
        sum(batch_outer(es, embedding_means) for es in embedding_samples) / 
...
def gaussian_KL(cov1, cov2, mean_diff):
    ...
    d = mean_diff.size(-1)
    return 0.5 * (
        logd_term - d + trce_term + quad_term
    )

def mean_NLL(distribution, point_samples):
    return torch.sum(-distribution.log_prob(ps) for ps in point_samples)/len(point_samples)
