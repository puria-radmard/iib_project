...
run_path="/home/alta/BLTSpeaking/exp-pr450/lent_shell_scripts/acquisition_distillation/run_train_regression_100.sh"
txt_base="/home/alta/BLTSpeaking/exp-pr450/lent_logs/acquisition_distillation_100/lc_bce_distillation_log_${txt_base_tag}"
    ...
    do
        ...
        do
            ...
            do
                ...
                if [[ $training_mode == "lc_histogram_jacobian_bce" ]]
                then
                    $run_path "${txt_base}${i}.txt" $layer  $logit_bank_path "${training_mode}" "${un}" ${test_prop}  50
                fi
                i=$((i+1))
            done
        done
