...
touch $1
set LOG=$1
set TRAIN='/home/alta/BLTSpeaking/exp-pr450/lent_shell_scripts/baseline_posterior_distillation/train_regression_100.sh'

set CMD = `qsub -cwd -j yes -o $LOG -P esol -l qp=cuda-low -l gpuclass='*' -l osrel='*' -l hostname='\\!air206.eng.cam.ac.uk' -l hostname='\\!air209.eng.cam.ac.uk' $TRAIN $2 $3 $4 $5 $6 $7`
